...
for epoch in range(num_epoches):
    ...
    for i, data in enumerate(trainDataLoader):
        ...
        num_acc = (pre == y).sum().item() # 加了个.item()
        train_acc += num_acc # .data
        # backward
        optimzier.zero_grad()
        loss.backward()
        optimzier.step()
        if (i + 1) % 100 == 0:
            print('[{}/{}],train loss is:{:.6f},train acc is:{:.6f}'.format(i, len(trainDataLoader), train_loss / (i * batch_size),train_acc / (i * batch_size)))


    print('epoch:[{}],train loss is:{:.6f},train acc is:{:.6f}'.format(epoch, train_loss / (len(trainDataLoader) * batch_size), train_acc / (len(trainDataLoader) * batch_size)))
    ...
    for i, data in enumerate(testDataLoader):
        ...
        _, pre = torch.max(out, 1)
        num_acc = (pre == y).sum().item()   # 和原来比，加了.item()
        eval_acc += num_acc                 # 改了原来的.data
